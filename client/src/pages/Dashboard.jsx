...
const Dashboard = () => {
  ...
  const { boss, employee } = useSelector(state => state.dashboard);
  ...
  useEffect(() => {
    if (userRole === "admin" && boss) {
      setDashboardData(boss);
    } else if (userRole !== "admin" && employee) {
      setDashboardData(employee);
    }
    ...
    if (userRole === "admin") {
      return [
        {
          _id: "1",
          label: "TOTAL TASKS",
          total: dashboardData.stats.totalTasks || 0,
        ...
        },
        {
          ...
          total: dashboardData.stats.completedTasks || 0,
        ...
        },
        {
          _id: "3",
          label: "HIGH PRIORITY",
          total: dashboardData.stats.highPriorityTasks || 0,
        ...
        },
        {
          ...
          total: dashboardData.stats.totalEmployees || 0,
      ...
      return [
        ...
        },
        {
          _id: "4",
          label: "TO DO TASKS",
          total: dashboardData.stats.todoTasks || 0,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-red-200 dark:border-red-800/50 p-4">
              ...
              {dashboardData.overdueTasks.length > 3 && (
                <button className="w-full mt-4 text-sm text-red-600 dark:text-red-400 font-medium py-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded">
                  View all overdue tasks ({dashboardData.overdueTasks.length})
                </button>
              )}
            </div>
          )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            ...
            <div className="space-y-4">
              {(userRole === "admin" ? dashboardData.recentTasks : dashboardData.assignedTasks)
                ?.slice(0, 5)
                .map((task) => (
                <div 
                  key={task._id} 
            ...
            {((userRole === "admin" && dashboardData.recentTasks?.length === 0) || 
              (userRole !== "admin" && dashboardData.assignedTasks?.length === 0)) && (
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-sm font-medium text-gray-800 dark:text-white">{dashboardData.stats.todoTasks || 0}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    className="bg-red-500 h-2 rounded-full" 
                    style={{ width: `${((dashboardData.stats.todoTasks || 0) / (dashboardData.stats.totalTasks || 1)) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-sm font-medium text-gray-800 dark:text-white">{dashboardData.stats.inProgressTasks || 0}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    className="bg-yellow-500 h-2 rounded-full" 
                    style={{ width: `${((dashboardData.stats.inProgressTasks || 0) / (dashboardData.stats.totalTasks || 1)) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-sm font-medium text-gray-800 dark:text-white">{dashboardData.stats.reviewTasks || 0}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full" 
                    style={{ width: `${((dashboardData.stats.reviewTasks || 0) / (dashboardData.stats.totalTasks || 1)) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-sm font-medium text-gray-800 dark:text-white">{dashboardData.stats.completedTasks || 0}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
